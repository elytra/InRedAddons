...
public class LEDRenderer extends BlockEntityRenderer<LEDBlockEntity> {
	...
	public void render(LEDBlockEntity be, double x, double y, double z, float partialTicks, int destroyStage) {
		...
		Sprite side = MinecraftClient.getInstance().getSpriteAtlas().getSprite("inredaddons:block/led_side_glow");
		Sprite end = MinecraftClient.getInstance().getSpriteAtlas().getSprite("inredaddons:block/led_end_glow");
